...
export default function PlayerHand({ myCards, lastMove, roomStatus, selectedCards, setSelectedCards, isValidMove }) {
    ...
    const toggleCardSelection = (clickedCard) => {
        ...
        if (isSelected) {
            return setSelectedCardsWithToggle(prev => prev.filter(c => !(c.rank === clickedCard.rank && c.suit === clickedCard.suit)));
        }
        ...
        if (lastCombo) {
            ...
            if (lastCombo.type === 'SINGLE') {
                if (lastCombo.highestRank === 15) {
                    ...
                    if (myQuads.length === 4) return setSelectedCardsWithToggle(myQuads);
                }
                return setSelectedCardsWithToggle([clickedCard]);
            }
            ...
            if (['PAIR', 'TRIPLE', 'QUAD'].includes(lastCombo.type)) {
                ...
                if (sameRankCards.length >= requiredLength) return setSelectedCardsWithToggle(sameRankCards.slice(0, requiredLength));
                return setSelectedCardsWithToggle([clickedCard]);
            }
            ...
            if (lastCombo.type === 'STRAIGHT') {
                ...
                if (autoStraight.length === requiredLength) return setSelectedCardsWithToggle(autoStraight);

                if (selectedCards.length >= requiredLength) return setSelectedCardsWithToggle([clickedCard]);

                return setSelectedCardsWithToggle(prev => [...prev, clickedCard]);
            }
        }
